...
def create_dataframe(path):
    ...
    for filename in os.listdir(path):
        ...
        if os.path.isfile(filepath):
            ...
            df.drop(df.loc[df['VHI'] == -1].index)
            df = df[df["Week"].notna()]
            df = df.dropna(axis=1)

            df["Area"] = regions[int(filepath.split("_")[3]) - 1]
...
regions = ["АР Крим", "Вінницька", "Волинська", "Дніпропетровська", "Донецька", "Житомирська", "Закарпатська",
           ...
           "Харківська", "Хмельницька", "Черкаська", "Чернівецька", "Чернігівська"]
